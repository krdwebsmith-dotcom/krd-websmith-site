...
/* Prefill source: the query string when it survives, otherwise the handoff stashed by script.js.
   Some static hosts 301 "/x.html?q" to "/x" and drop the query, so we keep a same-session fallback. */
var pfq=location.search.slice(1);
if(!pfq){try{pfq=sessionStorage.getItem("krd:prefill")||""}catch(e){pfq=""}}
try{sessionStorage.removeItem("krd:prefill")}catch(e){}
const params=new URLSearchParams(pfq);
